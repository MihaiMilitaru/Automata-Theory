    ...
    for state in mappedItems:
      ...
      for i in range(len(sigma)):
        ...
        for state2 in mappedItems:
          if item in state2.split("--") and state != state2:
            goodNode = True
            break
        if goodNode:
          break
      else:
        if [state, 'F'] not in mappedItemsAttrib and [state, 'S', 'F'] not in mappedItemsAttrib and [state, 'F'] not in mappedItemsAttrib and [state, 'F', 'S'] not in mappedItemsAttrib:
          canEnd = False
          mappedItems.remove(state)
          newItem = 0
          for node in mappedItemsAttrib:
            if state == node[0] or state == node:
              newItem = node
              break
          mappedItemsAttrib.remove(newItem)
    if canEnd:
      break
